...
# This script starts a light node that is connected to a local celestia-app validator.
# Prerequisites:
...
CHAIN_ID="test"
CELESTIA_HOME="${HOME}/.celestia-light-${CHAIN_ID}"
...
createConfig() {
    echo "Initializing light node config files..."
    celestia light init --core.ip $CORE_IP
    echo "Initialized light node config files."
}

startCelestia() {
    echo "Waiting for 6 seconds before starting light node..."
    sleep 6
    echo "Starting celestia light node..."
    celestia light start --core.ip $CORE_IP
}
